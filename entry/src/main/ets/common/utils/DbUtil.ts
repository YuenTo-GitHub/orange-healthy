...
class DbUtil {
  ...
  update(obj: any, value: relationalStore.ValuesBucket, predicates: relationalStore.RdbPredicates): Promise<number> {
    return new Promise((resolve, reject) => {
      this.rdbStore.update(value, predicates, (err, rows) => {
        if (err) {
          ...
          Logger.debug('修改成功！修改行数：', rows.toString())
          resolve(rows)
        }
      })
    })
  ...
  selectForList<T>(predicates: relationalStore.RdbPredicates, columns: ColumnInfo[]): Promise<T[]> {
    return new Promise((resolve, reject) => {
